...
pub fn run_wizard(config_path: &Path, existing: Option<&Config>) -> Result<()> {
    let defaults = existing.map_or_else(
        || (RadioSection::default(), BridgeSection::default()),
        |c| (c.radio.clone(), c.bridge.clone()),
    );
    ...
    let freq_default = format!("{:.3}", radio.frequency as f64 / 1_000_000.0);
    ...
    let port = if port_str == "auto" || port_str.is_empty() {
        None
    } else {
        Some(port_str)
    };
